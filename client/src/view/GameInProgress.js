...
const GameInProgress = (props) => {
  ...
  const getPlayerId = (event) => {
    ...
    return null === player
      ? activePlayer
      : parseInt(player.getAttribute('data-player'), 10);
  };
  ...
  const setNextActivePlayer = () => {
    const nextActivePlayer =
      activePlayer < props.playerCount - 1 ? activePlayer + 1 : 0;
  ...
  return (
    <div
      className='scorekeeper-game'
      onClick={(event) => determineActivePlayer(event)}
    >
