...
const saveGameState = (state) =>
  window.localStorage.setItem(gameCacheId, JSON.stringify(state));
...
class App extends Component {
  ...
  loadGameData(gameInProgress) {
    try {
      const { players, playerCount, activePlayer } = JSON.parse(gameInProgress);
  ...
  resetScores() {
    if (
      !this.confirmReset('Are you sure you want to reset scores for this game?')
    ) {
      return;
    }
        ...
        <div className='game_under'>
          {initialized && playerCount > 0 ? (
            <div>
              ...
              />
              <ResetControls
                restart={this.startNewGame.bind(this)}
                reset={this.resetScores.bind(this)}
              />
            </div>
          ) : playerCount === 0 ? (
...
export { App as default, saveGameState, getGameState };
